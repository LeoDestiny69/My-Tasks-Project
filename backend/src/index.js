...
const pool = new Pool({
  ...
  host: process.env.DB_HOST, // ควรจะเป็น 'db' เมื่อรันใน Docker
...
app.get('/todos', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM todos ORDER BY created_at DESC'); // เรียงตามเวลาสร้าง
...
app.post('/todos', async (req, res) => {
  // รับ title, description, due_date, priority จาก request body
  ...
  try {
    const result = await pool.query(
      ...
      [title, description || null, due_date || null, priority || 'Medium'] // ใช้ null ถ้าไม่มีค่า
    );
...
app.put('/todos/:id', async (req, res) => {
  const { id } = req.params;
  // รับค่าที่ต้องการอัปเดต (อาจจะส่งมาบางฟิลด์)
  const { title, description, is_completed, due_date, priority } = req.body;

  try {
    // ดึงข้อมูล todo เดิมมาก่อน เพื่อใช้ค่าเดิมหากไม่มีการส่งค่าใหม่มา
    ...
    // กำหนดค่าที่จะอัปเดต โดยใช้ค่าใหม่ถ้ามี มิฉะนั้นใช้ค่าเดิม
    ...
    // ตรวจสอบความถูกต้องของข้อมูลที่อัปเดต (Validation)
